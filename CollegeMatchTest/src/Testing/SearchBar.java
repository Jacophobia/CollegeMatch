...
//        for (int i = colleges_to_recommend.size(); i > 0; i--){
//            if (colleges_to_recommend.get(i).getGPA() > (GPA + .5))
//                colleges_to_recommend.remove(i);
//        }
//        if (colleges_to_recommend.size() == 0) {return colleges_to_recommend;}


        // removes colleges with 5 higher ACT score than the user
        for (int i = colleges_to_recommend.size() - 1; i > 0; i--){
            if (colleges_to_recommend.get(i).getACT() > (ACT + 5))
                colleges_to_recommend.remove(i);
        }
        if (colleges_to_recommend.size() == 0) {return colleges_to_recommend;}
